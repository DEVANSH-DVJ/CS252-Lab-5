...
void experiment(bool enableCtsRts, std::string wifiManager) {
  ...
  // Call the create method of that object, asking it to create given number of
  // nodes
  nodes.Create(4);
  ...
  // Create propagation loss matrix. This is a matrix that defines the amount of
  // loss in the signal strength that happens due to the distance it travels
  ...
  // set default loss to 200 dB - so much loss that there's essentially no
  // signal coverage
  ...
  // set Propagation delay based on some constant speed (value built in the
  // library)
  ...
  // RemotStationManager - method for setting rate characteristics of the
  // channel
  ...
  // Assign "wifiChannel" we created earlier, to the channel of this helper
  // object
  ...
  // Set its type to Adhoc (as opposed to infrastructure, we are not simulating
  // APs
  wifiMac.SetType("ns3::AdhocWifiMac"); // use ad-hoc MAC

  // Now, finally create the actual devices, in the Device container called
  // devices
  // and "Install" the Phy and the Mac helper objects on the "nodes" created
  // earlier This "installation" returns the container of netDevices which which
  // actually
  ...
  // Call the SetBase function, which defines the network prefix and the subnet
  // mask In essence this command is saying that IP addresses in this network
  ...
  // OnOff source helper. An On off source generates packets for some fixed
  // amount of time, then is silent for some fixed amount of time and this cycle
  // continues
  ...
  // Now Install this helper object on ni, and add the returned object, an
  // application to the cbrApps container
  ...
  UdpEchoClientHelper echoClientHelperC(Ipv4Address("10.0.0.4"), echoPort);
  ...
  pingApps.Add(echoClientHelperA.Install(nodes.Get(1)));

  echoClientHelperC.SetAttribute("StartTime",
                                 TimeValue(Seconds((double)2 / 1000)));
  pingApps.Add(echoClientHelperC.Install(nodes.Get(2)));
...
int main(int argc, char **argv) {
  ...
  std::cout << "Wi-fi Scenario 2 with RTS/CTS enabled:\n";
